                            ...
                            <div class="col-xl-12 col-md-12 col-lg-12 col-12">
                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0 table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($datakebutuhankhusus as $no =>  $item)
                                            <tr class="@if ($this->isChecked($item->agamaid))
                                                table-primary
                                                @endif">
                                                <th class="text-right" scope="row">{{ $no + $datakebutuhankhusus->firstItem() }}</th>
                                                <td>
                                                    {{ !empty($item->kebutuhan_khusus) ? $item->kebutuhan_khusus:'' }}<br/>
                                                </td>
                                            </tr>
                                            @endforeach
                                ...
                                <div class="mt-3 row">
                                    <div class="col-xl-12 col-md-12 col-lg-12 col-12 ">
                                        @if ($datakebutuhankhusus->total() > 10)
                                        {{ $datakebutuhankhusus->links() }}
                                        @else
                                        Page : {{ $datakebutuhankhusus->currentPage() }} | Show {{ $datakebutuhankhusus->count() }} data
                                        of {{ $datakebutuhankhusus->total() }}
                                        @endif
                                    </div>
